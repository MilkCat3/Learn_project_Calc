...
class Calculator(QMainWindow):
    def __init__(self):
        ...
        # ИНЖЕНЕРНЫЙ РЕЖИМ: Скрываем панель при старте
        if hasattr(self.ui, 'engineering_panel'):
            self.ui.engineering_panel.setVisible(False)

        # ИНЖЕНЕРНЫЙ РЕЖИМ: Привязываем клик по меню "Вид -> Инженерный"
        if hasattr(self.ui, 'action_engineering'):
            self.ui.action_engineering.triggered.connect(self.toggle_engineering_mode)

        self.entry.textChanged.connect(self.log_entry_change)
        ...
        # ИНЖЕНЕРНЫЙ РЕЖИМ: Привязываем кнопку тангенса
        if hasattr(self.ui, 'btn_tan'):
            self.ui.btn_tan.clicked.connect(self.calculate_tan)

    def log_entry_change(self, text: str) -> None:
        """Выводит текущее состояние le_entry в консоль при любом изменении"""
        logging.debug(f"Текущее значение дисплея (le_entry): '{text}' [Длина: {len(text)}]")

    def toggle_engineering_mode(self) -> None:
        """Переключает видимость левой панели и подгоняет размер окна"""
        if hasattr(self.ui, 'engineering_panel'):
            is_visible = self.ui.engineering_panel.isVisible()
            self.ui.engineering_panel.setVisible(not is_visible)

            # Заставляем Qt пересчитать геометрию слоев и ужать/расширить окно
            self.adjustSize()

    def calculate_tan(self) -> None:
        """Вычисляет тангенс от значения на дисплее (в градусах)"""
        self.remove_error()
        self.clear_temp_if_equality()

        try:
            val = self.get_entry_num()

            # ЛОГ: фиксируем, от какого числа считаем тангенс
            logging.debug(f"Нажата кнопка 'tan'. Входное значение (градусы): {val}")

            # Переводим из градусов в радианы и считаем тангенс
            res = math.tan(math.radians(val))

            # Округляем до 10 знаков, чтобы избежать компьютерной погрешности (типа 0.999999999)
            result_str = self.remove_trailing_zeros(str(round(res, 10)))

            self.entry.setText(result_str)
    ...
    def math_operation(self, math_sign: str):
        temp = self.temp.text().strip()
        btn = self.sender()

        # logs
        if btn:
            logging.debug(f"Нажата кнопка операции: '{btn.text()}' (ID: {btn.objectName()})")
